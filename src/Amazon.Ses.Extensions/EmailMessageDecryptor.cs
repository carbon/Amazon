...
public sealed class EmailMessageDecryptor
{
    ...
    public async Task<byte[]> DecryptAsync(S3Object encryptedBlob)
    {
        ...
        var cekAlg = encryptedBlob.Properties["x-amz-cek-alg"];
        ...
        var encryptionContext = JsonSerializer.Deserialize<Dictionary<string, string>>(encryptedBlob.Properties["x-amz-matdesc"])!;

        byte[] envelopeKey = Convert.FromBase64String(encryptedBlob.Properties["x-amz-key-v2"]);
        byte[] envelopeIV  = Convert.FromBase64String(encryptedBlob.Properties["x-amz-iv"]);
        long contentLength = long.Parse(encryptedBlob.Properties["x-amz-unencrypted-content-length"]);
        int tagLength      = int.Parse(encryptedBlob.Properties["x-amz-tag-len"]);
