...
{
    private static readonly ObjectPool<List<string>> s_stringListPool   = ObjectPool.Create(new ObjectPoolListPolicy<string>());
    private static readonly ObjectPool<List<DbValue>> s_dbValueListPool = ObjectPool.Create(new ObjectPoolListPolicy<DbValue>());
    ...
    {
        var rentedStringList = s_stringListPool.Get();
        ...
        {
            s_stringListPool.Return(rentedStringList);
        }
    }

    private static DbValue[] ReadDbValueArray(ref Utf8JsonReader reader)
    {
        var rentedDbValueList = s_dbValueListPool.Get();
        ...
        {
            s_dbValueListPool.Return(rentedDbValueList);
        }
    }
